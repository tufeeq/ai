(()=>{
  ...
  function mountAppBar(){
    ...
    bar.innerHTML=`<div class="app-command-left"><button id="appRefresh" class="primary">↻ تحديث مباشر</button><button id="installApp" class="app-secondary" hidden>تثبيت التطبيق</button><span id="appSyncState" class="app-sync">فحص المصادر…</span></div><div class="app-command-right"><span class="kbd-hint">/ بحث</span><span class="kbd-hint">R تحديث</span></div>`;
  ...
  async function refreshHealth(){
    const specs=[
      ['Finviz / Market','./data/finviz.json','market'],
      ['Yahoo + News + SEC','./data/enrichment.json','enrichment'],
      ['Final Reconciliation','./data/final-reconciliation.json','reconciliation']
    ];
  ...
  function wireRouting(){
    document.addEventListener('click',e=>{const tr=e.target.closest?.('.stock-row');if(tr?.dataset?.ticker)history.replaceState(null,'',`#stock=${encodeURIComponent(tr.dataset.ticker)}`);});
    window.addEventListener('hashchange',()=>{const m=location.hash.match(/stock=([^&]+)/);if(m&&typeof window.openTAG8Drawer==='function')window.openTAG8Drawer(decodeURIComponent(m[1]).toUpperCase());});
    setTimeout(()=>window.dispatchEvent(new Event('hashchange')),1000);
  }
  ...
  window.addEventListener('DOMContentLoaded',()=>{mountAppBar();mountHealth();restoreFilters();wireFilters();wireRouting();wireKeyboard();wireInstall();registerSW();refreshHealth();setInterval(refreshHealth,60000);});
})();