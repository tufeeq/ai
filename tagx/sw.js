...
self.addEventListener('fetch',e=>{
 ...
 if(e.request.mode==='navigate'&&u.pathname.includes('/ai/tagx/')){
  e.respondWith(fetch(e.request,{cache:'no-store'}).then(async r=>{
   ...
   if(!injected.includes('extended-v16.js'))injected=injected.replace('</body>','<script src="./extended-v16.js?v=17"></script></body>');
   if(!injected.includes('ui-v17.js'))injected=injected.replace('</body>','<script src="./ui-v17.js?v=17"></script><script>document.title="TAGX 1.7 — Smart Cards & Watchlists";const e=document.querySelector(".ey");if(e)e.textContent="TAGX 1.7 · SMART CARDS & WATCHLISTS";</script></body>');
