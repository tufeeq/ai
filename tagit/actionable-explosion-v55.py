...
disc=read_json(DISCOVERY,{'rows':[]});disc_rows=disc.get('rows') or []
disc_symbols=[]
for r in sorted(disc_rows,key=lambda x:(0 if 'nano_low_float' in (x.get('_discoveryLanes') or []) else 1)):
    ...
    if s and s not in disc_symbols:disc_symbols.append(s)
symbols=[]
...
for sym,bars in raw.items():
    ...
    for day in sorted(by):
        ...
        day_open=reg[0]['o'];sess_hi=max(z['h'] for z in a);sess_gain=ret(sess_hi,prev_close);is20=sess_gain>=20;is50=sess_gain>=50;t20=first_cross_time(a,prev_close,20);t50=first_cross_time(a,prev_close,50)
        cum=0.;pvnum=0.;pvden=0.;day_rows=[]
        for i,z in enumerate(a):
            unique_observations.add((sym,z['t']));cum+=z['v'];typ=(z['h']+z['l']+z['c'])/3;pvnum+=typ*z['v'];pvden+=z['v']
            if i<12 or i%2:continue
            c=z['c'];move=ret(c,prev_close);gap=ret(day_open,prev_close)
            if not (.15<=c<=30) or not (-18<=move<7.0) or c*cum<25000:continue
            p=a[:i+1]
            ...
            recent=p[-13:];cl=np.asarray([x['c'] for x in recent],float);compression=(max(cl)/min(cl)-1)*100 if len(cl)>1 and min(cl)>0 else 0;logr=np.diff(np.log(np.maximum(cl,1e-9)));volat=float(np.std(logr)*100) if len(logr)>1 else 0
            last=p[-7:];hi=max(x['h'] for x in last);lo=min(x['l'] for x in last);closepos=(c-lo)/(hi-lo) if hi>lo else .5;vwap=pvnum/pvden if pvden else c;vwapd=ret(c,vwap);acc=r5-r10/2;longacc=r10-r30/3;pd=prior_daily[-1] if prior_daily else {'range':0.,'volume':cum}
            ...
            for zz in a[i+1:]:
                ...
                if hit_t is None and zz['h']>=c*1.10:hit_t=zz['t'];break
            ...
            actionable=bool(hit_t is not None and 15<=lead<=75 and mae_to_hit>=-5 and mfe10<6);too_fast=bool(hit_t is not None and lead<15);ignition=(rvbar>=1.5 or rvcum>=1.5 or r10>=1.5 or r5>=1.0 or (closepos>=.72 and vwapd>=0));hard_neg=bool((not actionable) and (ignition or too_fast or is20 or is50));lead20=(t20-z['t'])/60 if t20 and t20>z['t'] else None;lead50=(t50-z['t'])/60 if t50 and t50>z['t'] else None
        ...
        rr=(max(z['h'] for z in reg)/min(z['l'] for z in reg)-1)*100 if min(z['l'] for z in reg)>0 else 0;prior_daily.append({'range':rr,'volume':sum(z['v'] for z in reg)});prior_daily=prior_daily[-20:];prev_close=reg[-1]['c']

dates=sorted({x['day'] for x in rows});ia=max(1,int(.70*len(dates)));ib=max(ia+1,int(.85*len(dates)));td=set(dates[:ia]);cd=set(dates[ia:ib]);hd=set(dates[ib:]);train=[x for x in rows if x['day'] in td];cal=[x for x in rows if x['day'] in cd];hold=[x for x in rows if x['day'] in hd];train_em=[x for x in emergence if x['day'] in td];train_cases=[x for x in explosion_cases if x['day'] in td]

def balanced_subset(rr,target,seed,neg_ratio=8):
    pos=[x for x in rr if x[target]];hard=[x for x in rr if not x[target] and x.get('hardNeg')];easy=[x for x in rr if not x[target] and not x.get('hardNeg')];rnd=random.Random(seed);rnd.shuffle(hard);rnd.shuffle(easy);hcap=min(len(hard),max(len(pos)*5,4000));ecap=min(len(easy),max(len(pos)*max(1,neg_ratio-5),3000));return pos+hard[:hcap]+easy[:ecap]
def fit_binary(rr,target,seed,balanced=True):
    ...
    cnt=Counter(x['key'] for x in use);ew=np.asarray([1/max(1,cnt[x['key']]) for x in use],float);hardw=np.asarray([2.2 if (not x[target] and x.get('hardNeg')) else 1.0 for x in use],float);sc=StandardScaler().fit(X);Xs=sc.transform(X);pos=max(1,int(y.sum()));neg=max(1,len(y)-pos);classw=np.where(y==1,min(80,neg/pos),1.);w=ew*hardw*classw
    et=ExtraTreesClassifier(n_estimators=420,max_depth=13,min_samples_leaf=7,max_features=.72,class_weight='balanced_subsample',n_jobs=-1,random_state=seed).fit(X,y,sample_weight=ew*hardw);hg=HistGradientBoostingClassifier(max_iter=240,max_leaf_nodes=19,learning_rate=.035,l2_regularization=12,min_samples_leaf=28,random_state=seed+1).fit(X,y,sample_weight=w);lr=LogisticRegression(max_iter=1000,C=.07,class_weight='balanced',random_state=seed+2).fit(Xs,y,sample_weight=ew*hardw)
...
def enrich(rr):
    ...
    for x,a,b,c,d,s in zip(rr,pa,p20,p50,da,sim):out.append({**x,'pAction':float(a),'p20':float(b),'p50':float(c),'disAction':float(d),'archSim':float(s),'decisionScore':float(.68*a+.17*b+.08*c+.07*s)})
    return out
def first(xs):
    out=[];seen=set()
...
cp=enrich(cal);configs=[];dvals=np.asarray([x['decisionScore'] for x in cp],float);avals=np.asarray([x['pAction'] for x in cp],float);svals=np.asarray([x['archSim'] for x in cp],float);ds_grid=sorted(set([.45,.55,.65,.75,.85]+[round(float(np.quantile(dvals,q)),4) for q in (.80,.88,.92,.95,.97)]));pa_grid=sorted(set([.35,.50,.65,.80]+[round(float(np.quantile(avals,q)),4) for q in (.80,.90,.95)]));sim_grid=sorted(set([0,.20,.40]+[round(float(np.quantile(svals,q)),4) for q in (.25,.50,.75)]))
for ds in ds_grid:
  for pa in pa_grid:
    for p20 in (0,.15,.30):
      for dis in (.10,.18,.28):
        for sim in sim_grid:
          for ss in ((0,1),(1,),(0,1,2)):
            cfg=(ds,pa,p20,dis,sim,ss);z=stat(pick(cp,cfg),cp);n=z['count'];p=z['precisionActionablePct'] or 0;lo=z['wilsonLower90Pct'] or 0;credible=n>=30;utility=(1_000_000 if (credible and p>=90) else 0)+lo*100+p*10+(z['tp']*2)+(z['tickerDayRecallPct'] or 0)
            if n<15:utility-=5000
            configs.append((utility,cfg,z))
configs.sort(key=lambda x:x[0],reverse=True);_,cfg,cm=configs[0];hp=enrich(hold);hs=stat(pick(hp,cfg),hp)
