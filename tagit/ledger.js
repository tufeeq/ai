...
function rows(){
  ...
  return[...document.querySelectorAll('#rows tr[data-symbol]')].map(r=>{
    const c=r.children,d=r.dataset;
    return{
      symbol:d.symbol,
      phase:d.phase||c[1]?.textContent?.trim(),
      score:num(d.score??c[3]?.textContent),
      actionability:num(d.actionability??c[4]?.textContent),
      gate:(d.gate||c[1]?.textContent||'').trim().toUpperCase(),
      price:num(d.price??c[8]?.textContent)
    }
...
function ingest(){
  ...
  for(const p of q.filter(x=>x.gate==='WATCH'))if(!ledger.some(x=>x.symbol===p.symbol&&!x.closedAt&&now-x.openedAt<6*HOUR))ledger.push({id:`${p.symbol}-${now}`,symbol:p.symbol,openedAt:now,entryPrice:p.price,phase:p.phase,score:p.score,actionability:p.actionability,mfe:0,mae:0,source:'TAGit-v5-row-contract'});
  save(ledger);render();
}
