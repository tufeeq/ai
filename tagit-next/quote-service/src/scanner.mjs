...
export function createScanner({env=process.env,fetcher=fetch,now=Date.now,onEvidence=null}={}){
 ...
 async function scan(){
  ...
  cached={schema_version:1,status:failed||historyError?'PARTIAL':'OK',server_time:new Date(checked).toISOString(),scan_started_at:new Date(started).toISOString(),refresh_ms:30000,feed:s.feed,coverage:{nasdaq_assets:universe.listed,eligible_small_caps:symbols.length,with_prices:rows.length,fresh_prices:rows.filter(r=>r.status==='FRESH').length,failed_symbols:failed,detailed_symbols:shortlist.length,metadata_at:universe.metadata_at,max_market_cap_exclusive:100000000,scope:'NASDAQ equities below $100M present in reference; excludes missing metadata and funds',history_error:historyError,news_error:newsError,news_at:newsAt?new Date(newsAt).toISOString():null},rules:RULES,rows:[...new Map([...rows.sort((a,b)=>Number(a.extended)-Number(b.extended)||b.score-a.score||(b.day_change??-Infinity)-(a.day_change??-Infinity)).slice(0,150),...gainers.slice(0,50)].map(r=>[r.symbol,r])).values()],gainers:gainers.slice(0,50).map(r=>r.symbol),alerts:ledger,storage:'PROCESS_MEMORY',strategy_validation:'UNPROVEN',coverage_note:s.feed==='iex'?'IEX single exchange; not the entire US market':'Consolidated feed'};if(onEvidence)onEvidence({scan:cached,histories});return cached;
 }
