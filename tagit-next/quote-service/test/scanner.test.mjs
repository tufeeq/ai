...
test('whole scanner intersects verified NASDAQ assets with fresh small-cap metadata and caches scans',async()=>{
 ...
 const fetcher=async url=>{calls++;let payload;if(url.includes('/assets?'))payload=[{symbol:'TEST',exchange:'NASDAQ',status:'active'},{symbol:'NYSE',exchange:'NYSE',status:'active'}];else if(url.includes('universe-broad'))payload={schemaVersion:1,updatedAt:new Date(now).toISOString(),rows:[{Ticker:'TEST',Company:'Test',Industry:'Software','Market Cap':'10'}]};else if(url.includes('/snapshots?'))payload={TEST:{latestTrade:{p:2.04,t:new Date(now).toISOString()},latestQuote:{bp:2.039,ap:2.04,t:new Date(now).toISOString()},dailyBar:{c:2.04,v:100000,h:2.05},prevDailyBar:{c:2}}};else if(url.includes('/bars?'))payload={bars:{TEST:bars}};else payload={news:[]};return{ok:true,json:async()=>payload};};
 const scanner=createScanner({env:{ALPACA_API_KEY_ID:'test',ALPACA_API_SECRET_KEY:'test'},fetcher,now:()=>now});const r=await scanner.get();assert.equal(r.coverage.eligible_small_caps,1);assert.equal(r.rows[0].symbol,'TEST');assert.equal(r.rows[0].signal.expansion,true);assert.equal(r.alerts.length,1);const count=calls;await scanner.get();assert.equal(calls,count);
});
