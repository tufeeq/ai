...
async function loadPublished() {
  const [enrichment, relabel, forward, sip, exits, filters] = await Promise.all([
  ...
  state.evidence = { relabel: relabel?.corrected ? relabel : null, forward: forward?.days ? forward : null, sip: sip?.totals ? sip : null, exits: exits?.development ? exits : null, filters: filters?.baseline ? filters : null };
