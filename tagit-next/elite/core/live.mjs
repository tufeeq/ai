...
export function createElite({env=process.env,fetcher=fetch,now=Date.now}={}) {
 ...
 const status={mode:'SHADOW',version:config.version,release:env.RENDER_GIT_COMMIT||'LOCAL',configHash:hash(config),background:env.TAGIT_BACKGROUND==='1',durabilityVerified:false,
   ...
   lastObservedAt:null,lastError:null,busy:false,processedBars:0,duplicateBars:0,invalidBars:0,computeMs:null,calendarSource:'PROVIDER',approvedForLive:false};
 ...
 async function consume({scan,histories}) {
  if(stopped||scan.server_time===lastScan)return;status.busy=true;const started=now();
  try{
   ...
   const tracked=engine.snapshots().filter(o=>o.session===day).map(o=>o.symbol),missing=tracked.filter(s=>!histories[s]);
 ...
 return {observe(e){if(!status.busy&&!stopped)chain=chain.then(()=>consume(e)).catch(()=>{status.lastError='STORAGE_FAILED';});},
  ...
  snapshot(){const rows=store.snapshots(runId);return {name:'TAG elite',status:this.status(),mode:'SHADOW',serverTime:new Date(now()).toISOString(),opportunities:rows.map(o=>({...o,lastTrade:livePrices.get(o.symbol)??null,timeline:store.timeline(o.id)})),rulesPromoted:false};},
