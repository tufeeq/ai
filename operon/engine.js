...
function detectDecisions(s){
 ...
 if(s.overdueAR>=Math.max(25000,rev*.05)) addDecision(d,{domain:'Finance',severity:arR>.25?'critical':'high',title:'Accelerate overdue collections',impact:s.overdueAR,action:'Start risk-tiered collection sequence',confidence:clamp(Math.round(78+arR*40),78,97)},.32);
 if(s.cashRunwayDays<60) addDecision(d,{domain:'Finance',severity:s.cashRunwayDays<30?'critical':'high',title:'Protect operating cash runway',impact:Math.round(rev*.15),action:'Model and delay non-critical spend',confidence:91},.50);
 ...
 if(s.stalePipeline>=Math.max(50000,rev*.10)) addDecision(d,{domain:'Revenue',severity:ratio(s.stalePipeline,rev)>.75?'high':'medium',title:'Recover stale pipeline',impact:s.stalePipeline,action:'Launch prioritized personalized follow-up',confidence:86},.12);
 if(s.vendorLeakage>=Math.max(15000,rev*.02)) addDecision(d,{domain:'Procurement',severity:ratio(s.vendorLeakage,rev)>.10?'high':'medium',title:'Consolidate supplier spend',impact:s.vendorLeakage,action:'Open supplier consolidation review',confidence:82},.75);
 if((s.churnRiskPct||0)>=35&&(s.revenueAtRisk||0)>=Math.max(25000,rev*.05)) addDecision(d,{domain:'Customer',severity:s.churnRiskPct>=65?'critical':'high',title:'Prevent customer churn',impact:s.revenueAtRisk,action:'Launch executive recovery plan',confidence:clamp(Math.round(70+s.churnRiskPct*.25),75,95)},.30);
 if((s.overCapacityPct||0)>=20&&(s.capacityRevenueAtRisk||0)>=Math.max(25000,rev*.04)) addDecision(d,{domain:'People',severity:s.overCapacityPct>=40?'critical':'high',title:'Resolve capacity bottleneck',impact:s.capacityRevenueAtRisk,action:'Rebalance workload or add temporary capacity',confidence:84},.35);
