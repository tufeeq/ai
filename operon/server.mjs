...
import {emptyState,seedState,apply,can,VERSION} from './core.mjs';
...
`);
for(const [column,type] of [['created','INTEGER'],['last_seen','INTEGER'],['agent','TEXT']])if(!db.prepare('PRAGMA table_info(sessions)').all().some(r=>r.name===column))db.exec('ALTER TABLE sessions ADD COLUMN '+column+' '+type);
...
function issueSession(res,user,req){const token=randomBytes(32).toString('hex'),csrf=randomBytes(24).toString('hex');db.prepare('INSERT INTO sessions(token,user,csrf,expires,created,last_seen,agent) VALUES(?,?,?,?,?,?,?)').run(sha(token),user,csrf,Date.now()+7*86400000,Date.now(),Date.now(),String(req.headers['user-agent']||'Unknown client').slice(0,250));const secure=process.env.OPERON_SECURE_COOKIES==='1';res.setHeader('Set-Cookie',`operon_session=${token}; Path=/; HttpOnly; SameSite=Strict; Max-Age=604800${secure?'; Secure':''}`);return csrf;}
function session(req){const raw=(req.headers.cookie||'').split(';').map(x=>x.trim()).find(x=>x.startsWith('operon_session='))?.slice(15);const row=raw&&db.prepare('SELECT s.*,u.name,u.email FROM sessions s JOIN users u ON s.user=u.id WHERE token=? AND expires>?').get(sha(raw),Date.now());check(row,'Sign in to continue',401);db.prepare('UPDATE sessions SET last_seen=? WHERE token=?').run(Date.now(),row.token);return row;}
...
function adminAudit(id,actor,action,detail){const s=workspace(id);s.audit.unshift({id:randomUUID(),at:new Date().toISOString(),actor,action,entity:'administration',detail});s.version++;save(id,s);}
...
const limits=new Map();function rate(req,account){const key=account?'account:'+account:req.socket.remoteAddress,now=Date.now();let r=limits.get(key);if(!r||r.until<now){r={count:0,until:now+600000};limits.set(key,r);}check(++r.count<=30,'Too many attempts. Try again in ten minutes.',429);}
...
const staticFiles={'/':'index.html','/welcome.html':'welcome.html','/marketing.css':'marketing.css','/marketing.mjs':'marketing.mjs','/index.html':'index.html','/app.mjs':'app.mjs','/core.mjs':'core.mjs','/styles.css':'styles.css','/favicon.svg':'favicon.svg'};
...
const server=http.createServer(async(req,res)=>{
 if(process.env.OPERON_SECURE_COOKIES==='1')res.setHeader('Strict-Transport-Security','max-age=31536000');res.setHeader('Permissions-Policy','camera=(), microphone=(), geolocation=()');
 ...
 try{
  ...
  if(req.method==='POST'&&['/api/register','/api/login'].includes(path)){
   rate(req);const b=await body(req),email=String(b.email||'').trim().toLowerCase(),password=String(b.password||'');check(email.length<255&&/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email),'Valid email required');check(password.length>=12&&password.length<=256,'Use a password of 12–256 characters');rate(req,email);let user;
   ...
   else {const row=db.prepare('SELECT * FROM users WHERE email=?').get(email);const valid=passwordOK(password,row?.password||dummyPassword);check(row&&valid,'Invalid email or password',401);user=row.id;}
  ...
  if(path==='/api/sessions'&&req.method==='GET'){json(res,200,{sessions:db.prepare('SELECT token,created,last_seen,expires,agent FROM sessions WHERE user=? AND expires>?').all(sess.user,Date.now()).map(r=>({id:sha(r.token+'session-id').slice(0,20),current:r.token===sess.token,created:r.created,lastSeen:r.last_seen,expires:r.expires,agent:r.agent}))});return;}
  if(path==='/api/sessions/revoke'&&req.method==='POST'){const b=await body(req);if(b.others===true)db.prepare('DELETE FROM sessions WHERE user=? AND token<>?').run(sess.user,sess.token);else{const target=db.prepare('SELECT token FROM sessions WHERE user=?').all(sess.user).find(r=>sha(r.token+'session-id').slice(0,20)===b.id);check(target&&target.token!==sess.token,'Other session not found',404);db.prepare('DELETE FROM sessions WHERE token=? AND user=?').run(target.token,sess.user);}json(res,200,{ok:true});return;}
  ...
  if(path==='/api/invites/accept'&&req.method==='POST'){const b=await body(req);transaction(()=>{const i=db.prepare('SELECT * FROM invites WHERE token=? AND expires>?').get(sha(String(b.token)),Date.now());check(i&&i.email===sess.email,'Invite is invalid, expired, or for a different account',403);db.prepare('INSERT OR IGNORE INTO members VALUES(?,?,?)').run(i.workspace,sess.user,i.role);db.prepare('DELETE FROM invites WHERE token=?').run(i.token);adminAudit(i.workspace,sess.name,'Invitation accepted',sess.email+'; role '+i.role);});json(res,200,{workspaces:list(sess.user)});return;}
  const match=path.match(/^\/api\/workspaces\/([^/]+)(?:\/(command|members|invites|export|admin|invite-revoke))?$/);check(match,'Endpoint not found',404);const id=match[1],action=match[2],m=member(sess.user,id);
  if(req.method==='GET'&&!action){json(res,200,{state:workspace(id),role:m.role});return;}
  if(req.method==='GET'&&action==='admin'){check(m.role==='owner','Owner access required',403);json(res,200,{invites:db.prepare('SELECT token,email,role,expires FROM invites WHERE workspace=? AND expires>?').all(id,Date.now()).map(i=>({id:sha(i.token+'invite-id').slice(0,20),email:i.email,role:i.role,expires:i.expires})),jobs:db.prepare('SELECT started,status,error FROM jobs WHERE workspace=? ORDER BY id DESC LIMIT 20').all(id),secureCookies:process.env.OPERON_SECURE_COOKIES==='1',signupOpen:process.env.OPERON_ALLOW_SIGNUP!=='0'});return;}
  if(req.method==='POST'&&action==='invite-revoke'){check(m.role==='owner','Owner access required',403);const b=await body(req);transaction(()=>{const target=db.prepare('SELECT token,email FROM invites WHERE workspace=?').all(id).find(i=>sha(i.token+'invite-id').slice(0,20)===b.id);check(target,'Invitation not found',404);db.prepare('DELETE FROM invites WHERE token=? AND workspace=?').run(target.token,id);adminAudit(id,sess.name,'Invitation revoked',target.email);});json(res,200,{ok:true});return;}
  ...
  if(req.method==='POST'&&action==='invites'){
   check(m.role==='owner','Only the owner can invite members',403);const b=await body(req),email=String(b.email||'').trim().toLowerCase();check(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email),'Valid email required');check(['manager','operator','viewer'].includes(b.role),'Invalid role');const token=randomBytes(32).toString('hex');transaction(()=>{db.prepare('INSERT INTO invites VALUES(?,?,?,?,?)').run(sha(token),id,email,b.role,Date.now()+7*86400000);adminAudit(id,sess.name,'Invitation created',email+'; role '+b.role);});json(res,201,{token,expiresInDays:7,note:'Share this invitation token with the intended person. No email was sent.'});return;
  }
  if(req.method==='POST'&&action==='members'){
   check(m.role==='owner','Only the owner can change membership',403);const b=await body(req);check(b.user!==sess.user,'Owner cannot remove their own access');const target=member(b.user,id);check(target.role!=='owner','Owner membership cannot be changed');transaction(()=>{if(b.remove)db.prepare('DELETE FROM members WHERE workspace=? AND user=?').run(id,b.user);else{check(['manager','operator','viewer'].includes(b.role),'Invalid role');db.prepare('UPDATE members SET role=? WHERE workspace=? AND user=?').run(b.role,id,b.user);}adminAudit(id,sess.name,b.remove?'Membership removed':'Membership role changed',b.user+'; '+(b.remove?'removed':b.role));});json(res,200,{ok:true});return;
  }
  if(req.method==='POST'&&action==='command'){
   const b=await body(req),key=req.headers['idempotency-key'];check(typeof key==='string'&&key.length>=8&&key.length<=200,'Idempotency key required');check(m.role!=='viewer','Read-only workspace',403);const signature=sha(sess.user+':'+JSON.stringify(b));
