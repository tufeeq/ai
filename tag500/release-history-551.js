...
(function(){
  ...
  function ensureStyles(){
    ...
    const s=document.createElement('style');s.id='tag558-release-history-style';
  ...
  async function fetchJSON(url){const r=await fetch(url+(url.includes('?')?'&':'?')+'ts='+Date.now(),{cache:'no-store'});if(!r.ok)throw new Error('HTTP '+r.status+' '+url);return r.json();}
  function mergeReleases(base,journal){
    const byVersion=new Map();
    for(const r of [...(Array.isArray(base)?base:[]),...(Array.isArray(journal)?journal:[])])if(r?.version)byVersion.set(String(r.version),r);
    return [...byVersion.values()].sort((a,b)=>(Number(String(a.version).replace(/\D/g,''))||0)-(Number(String(b.version).replace(/\D/g,''))||0));
  }
  function render(data){
    const box=host();if(!box)return;ensureStyles();
    const current=String(data?.current||releaseFromPage());setRelease(current);
    const note=box.querySelector('.release-note');if(note&&data?.currentSummary)note.textContent=`${current}: ${String(data.currentSummary)}`;
    const releases=Array.isArray(data?.releases)?data.releases:[];
    let region=box.querySelector('[data-release-history]');if(!region){region=document.createElement('div');region.dataset.releaseHistory='1';box.appendChild(region);}
  ...
  function installFinalStateGate(){
    ...
    window.render=function(){
      const result=baseRender.apply(this,arguments),state=window.TAG500State&&typeof window.TAG500State==='object'?window.TAG500State:{};
      const analyzed=Array.isArray(window.analyzed)?window.analyzed:(Array.isArray(state.analyzed)?state.analyzed:[]),rows=Array.isArray(window.rows)?window.rows:(Array.isArray(state.rows)?state.rows:[]);
      generation+=1;const build=releaseFromPage();
      window.TAG500State={...state,build,phase:'FINAL',finalGeneration:generation,rows,analyzed,analyzedCount:analyzed.length,finalizedAt:new Date().toISOString()};
      window.dispatchEvent(new CustomEvent('tag500:state-final',{detail:{build,finalGeneration:generation,analyzedCount:analyzed.length}}));
      return result;
    };
  }
  function verifyRuntimeOwnership(current){
    const n=Number(String(current).replace(/\D/g,''))||0;
    const scripts=[...document.scripts].map(s=>s.getAttribute('src')||'');
    const duplicateFinal=scripts.filter(src=>/final-snapshot-authority-\d+\.js/.test(src));
    const ok=duplicateFinal.length<=1;
    window.TAG500ReleaseIntegrity={build:current,entrypointOwnsRuntime:true,dynamicRuntimeInjection:false,finalAuthorityScriptCount:duplicateFinal.length,ok};
    const log=document.querySelector('#integrityLog');
    if(log&&!ok){const d=document.createElement('div');d.className='log-item warn';d.textContent=`Release Integrity: duplicate Final Authority scripts (${duplicateFinal.length})`;log.appendChild(d);}
    window.dispatchEvent(new CustomEvent('tag500:runtime-ready',{detail:{layer:'releaseHistory',build:current,releaseIntegrity:ok}}));
    return n;
  }
  async function load(){
    const box=host();if(!box)return;
    try{
      const [ledger,currentMeta,journal]=await Promise.all([fetchJSON(LEDGER_URL),fetchJSON(CURRENT_URL).catch(()=>null),fetchJSON(JOURNAL_URL).catch(()=>({releases:[]}))]);
      const releases=mergeReleases(ledger?.releases,journal?.releases);
      const current=String(currentMeta?.current||ledger?.current||releaseFromPage());
      if(currentMeta?.current&&!releases.some(r=>String(r.version)===current))releases.push({version:current,date:String(currentMeta.updatedAt||'').slice(0,10),summary:String(currentMeta.summary||'')});
      render({...ledger,current,releases,currentSummary:String(currentMeta?.summary||'')});
      verifyRuntimeOwnership(current);
    }catch(e){
      ensureStyles();let region=box.querySelector('[data-release-history]');if(!region){region=document.createElement('div');region.dataset.releaseHistory='1';box.appendChild(region);}
  ...
  setRelease(releaseFromPage());
  installFinalStateGate();
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',load);else load();
  window.TAG500ReleaseHistory={get build(){return releaseFromPage();},load,render,mergeReleases,releaseAuthority:'entrypoint-runtime-current-release-identity',dynamicRuntimeInjection:false};
})();
