...
(function(){
  ...
  function trainingReason(z){
    const m=window.sourceMeta||{},recon=String(m.reconciliation||m.finalSnapshotReconciliation||'').toUpperCase(),sources=Number(m.independentSourceCount||0);
    if(z?.dataComplete===false) return 'Training: مدخلات السهم ناقصة';
    if(sources<2) return `Training: مصادر مستقلة ${sources}/2`;
    if(!['RECONCILED','MATCHED','FINAL_RECONCILED','VERIFIED_FINAL'].includes(recon)) return 'Training: Final Reconciliation pending';
    if(m.trainingEligible===false) return 'Training: محجوب بجودة البيانات';
    return 'Training ✓';
  }
  function whyNow(z){
    const bits=[],t=temporal(z),c=catalyst(z),o=origin(z);
    if(['EARLY','FORMING'].includes(o)) bits.push(`أصل الإشارة ${o==='EARLY'?'مبكر':'يتكوّن'}`);
  ...
  function decisionCmp(a,b){
    ...
    const cmp=window.TAG500Actionability?.cmp;
    if(typeof cmp==='function') return cmp(a,b);
  ...
  function card(z,rank){
    const yes=eligible(z),train=training(z),now=whyNow(z),no=blockers(z),tr=trainingReason(z);
    return `<article class="dt-card ${yes?'dt-ok':'dt-watch'}"><div class="dt-head"><div><span class="dt-rank">#${rank}</span> <strong>${esc(z.ticker)}</strong> <span class="dt-stage">${esc(z.stage||'—')}</span></div><strong>${Number.isFinite(num(z.score))?Math.round(num(z.score)):'—'}</strong></div><div class="dt-meta"><span>${pct(z.changePct)}</span><span>${esc(origin(z))}</span><span>Action ${esc(action(z))}</span><span>${yes?'تنفيذي':'بحث/مراقبة'}</span><span>${train?'Training ✓':esc(tr)}</span></div><div class="dt-cols"><div><b>لماذا الآن</b><p>${now.length?now.map(esc).join(' · '):'لا يوجد تأكيد مبكر كافٍ بعد'}</p></div><div><b>ما الذي يمنعها</b><p>${no.length?no.map(esc).join(' · '):'لا يوجد مانع تنفيذي رئيسي ظاهر'}</p></div></div></article>`;
  }
  function renderTrace(){
    const all=Array.isArray(window.analyzed)?window.analyzed:[];let host=document.querySelector('#decisionTrace545')||document.querySelector('#decisionTrace544')||document.querySelector('#decisionTrace543');
    if(!host){host=document.createElement('section');host.className='panel decision-trace';const a=document.querySelector('#radarSection')||document.querySelector('#opportunities');a?.parentNode?.insertBefore(host,a?.nextSibling||null);}host.id='decisionTrace545';
    const ranked=[...all].sort(decisionCmp).slice(0,3);
    ...
    host.innerHTML=`<div class="section-head"><div><h3>مسار القرار — Decision Trace</h3><p>نفس ترتيب Actionability المستخدم في «أفضل فرصة»؛ Catalyst Clock يكسر التعادل داخل الفئة نفسها. أهلية التدريب مستقلة وتظل Final-Reconciliation gated.</p></div><span class="dt-status">${status}</span></div><div class="dt-grid">${ranked.length?ranked.map((z,i)=>card(z,i+1)).join(''):'<div class="empty">لا توجد حالات قابلة للعرض بعد.</div>'}</div>`;
  }
  const baseRender=window.render;if(typeof baseRender==='function'&&!baseRender.__tag545){const wrapped=function(){const r=baseRender.apply(this,arguments);queueMicrotask(renderTrace);return r;};wrapped.__tag545=true;window.render=wrapped;}
  ...
  window.TAG500DecisionTrace={release:RELEASE,render:renderTrace,compare:decisionCmp};
})();