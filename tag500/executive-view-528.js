...
(function(){
  ...
  function paintVersion(){
    ...
    if(release){
      ...
      if(chips)chips.innerHTML='<span class="release-chip">2026-08-17</span><span class="release-chip">Final-close boundary integrity</span><span class="release-chip">Same-session guard</span><span class="release-chip">Fail-closed preserved</span><span class="release-chip">No threshold retune</span>';
      const note=release.querySelector('.release-note');
      if(note)note.textContent='TAG531: أصلح Final Close Gate عند حد 8:00 PM ET، فأصبحت لقطة 8:00+ مؤهلة كمرشح إغلاق نهائي إذا كانت من نفس جلسة اليوم، ومنع اعتبار لقطة AH قديمة من يوم سابق إغلاقًا نهائيًا. لا تغيير في thresholds أو scoring.';
    }
  }
  ...
  function apply(){
    ...
    if(item)item.innerHTML=health.ok?`Runtime ${BUILD}: ✓ الطبقات المطلوبة محمّلة؛ Final Close Gate يتحقق من حد 8:00 PM ET ومن تطابق جلسة المصدر مع جلسة اليوم.`:`Runtime ${BUILD}: ⚠ طبقات مفقودة: ${health.missing.join(', ')}. Executive Mode محجوب مؤقتًا دون تعديل z.valid.`;
