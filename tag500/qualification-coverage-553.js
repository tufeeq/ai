...
(function(){
  ...
  function shariaOk(z){return String(z?.shariaStatus||z?.sharia?.status||'').toUpperCase()==='VERIFIED';}
  ...
  function snapshot(){
    ...
    return {total:a.length,sharia:a.filter(shariaOk).length,catalyst:a.filter(catalystReady).length,model:a.filter(modelReady).length,early:a.filter(earlyOrigin).length,executive:a.filter(executive).length,sourceMeasured,top};
  }
  ...
  function renderCoverage(){
    ...
    if(!panel){
      panel=document.createElement('section'); panel.id='qualificationCoverage553'; panel.className='panel';
      host.parentNode.insertBefore(panel,host);
    }
    const s=snapshot();
    const primary=s.top[0];
    const bottleneck=primary?`${primary[0]} (${primary[1]})`:'لا يوجد مانع موحد';
    panel.innerHTML=`<div class="section-head"><div><h3>تغطية أهلية القرار · ${BUILD}</h3><p>تفصل بين جودة الاكتشاف وجودة المدخلات. هذه المقاييس تشخيصية فقط ولا تغيّر score أو thresholds.</p></div></div>
