...
(function(){
  ...
  function sourceName(){
    return window.__TAG500_DATA_SOURCE__?.label||window.__TAG500_DATA_SOURCE__?.mode||window.sourceMeta?.name||'غير معروف';
  }
  function health(){
    const age=ageMinutes();
    const stale=age===null||age>STALE_MIN;
    return {build:BUILD,busy,lastAttempt,lastSuccess,lastError,failures,ageMinutes:age,stale,source:sourceName()};
  }
  function paint(reason){
    const h=health();
    const badge=document.querySelector('#dataBadge');
    const note=document.querySelector('#refresh517Note');
    if(badge){
      badge.title=`${BUILD} · source=${h.source} · age=${h.ageMinutes===null?'?':h.ageMinutes.toFixed(1)+'m'} · failures=${h.failures}`;
      if(h.stale||h.failures>0) badge.classList.remove('live');
    }
    if(note){
      ...
      const state=h.stale?'⚠ البيانات قديمة/غير مؤرخة':h.failures?'⚠ آخر تحديث فشل':'✓ التحديث سليم';
      note.innerHTML=`<strong>${state}</strong> · عمر اللقطة ${age} · آخر نجاح ${ok} · آخر محاولة ${attempt} · المصدر ${h.source}${h.failures?` · إخفاقات متتالية ${h.failures}`:''}${reason?' · '+reason:''}${h.lastError?`<br><small>${String(h.lastError).replace(/[&<>]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[m]))}</small>`:''}`;
    }
  }
  async function trackedLoad(){
    ...
    try{
      ...
      failures+=1;lastError=e?.message||String(e);
      paint('فشل التحديث');
      throw e;
    }
  }
  ...
  function mount(){
    const old=document.querySelector('#refresh516Note');if(old)old.remove();
    if(!document.querySelector('#refresh517Note')){
      const status=document.querySelector('#finvizStatus');
      if(status){
        const n=document.createElement('div');n.id='refresh517Note';n.className='release-note';n.style.marginTop='6px';n.textContent=BUILD+' · فحص صحة التحديث مفعّل';status.after(n);
      }
    }
  ...
  window.TAG500RefreshHealth={getState:health,refresh:guardedRefresh};
