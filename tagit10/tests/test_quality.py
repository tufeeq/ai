...
class QualityTests(unittest.TestCase):
 def setUp(self):
  ...
  self.row={'symbol':'TEST','stage':'EARLY','quoteFresh':True,'screeningPassed':True,'barClosed':True,'price':100,'changePct':2,'score':55,'quoteTimestampUTC':self.at.isoformat()}
 ...
 def test_stop_before_target_is_failure(self):
  sig=self.freeze();self.row['_points']=[(self.at.timestamp()+i*60,97 if i==3 else 104,100,100,104 if i!=3 else 100,97 if i==3 else 100) for i in range(1,31)]
  # First two closes must not already hit target.
  self.row['_points'][0]=(self.at.timestamp()+60,100,100,100,100,100);self.row['_points'][1]=(self.at.timestamp()+120,100,100,100,100,100)
 ...
 def test_target_before_stop(self):
  sig=self.freeze();self.row['_points']=[(self.at.timestamp()+i*60,104 if i==3 else 100,100,100,104 if i==3 else 100,100) for i in range(1,31)];evaluate_signals(self.state,[self.row],(self.at+timedelta(minutes=32)).isoformat());self.assertEqual(sig['label'],'TARGET_FIRST')
