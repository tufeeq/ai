...
STATE_PATH=ROOT/'tagit10-state.json'; OUT_PATH=ROOT/'tagit10-live.json'; TOKEN=os.getenv('FINVIZ_TOKEN','').strip(); UA='Mozilla/5.0 TAGit10/1.1'
...
def save_state(s):
    ...
    if len(s.get('symbols',{}))>2500:
        s['symbols']=dict(sorted(s['symbols'].items(),key=lambda kv:(kv[1].get('maxScore',0),kv[1].get('lastSeenUTC','')),reverse=True)[:2500])
    ...
    for sig in ('ta_topgainers','ta_unusualvolume','ta_mostactive'):
        try:
            ...
            for r in csv.DictReader(io.StringIO(raw)):
                ...
                if not s:continue
                z=merged.setdefault(s,{}); z.update(r); z.setdefault('_lanes',[]).append(sig)
        except:pass
        time.sleep(.5)
...
def symbol_directory():
    ...
    for u in ('https://www.nasdaqtrader.com/dynamic/SymDir/nasdaqlisted.txt','https://www.nasdaqtrader.com/dynamic/SymDir/otherlisted.txt'):
        try:
            rows=list(csv.DictReader(io.StringIO(get(u,15).decode('utf-8','ignore')),delimiter='|'))
            for r in rows:
                s=(r.get('Symbol') or r.get('ACT Symbol') or '').strip().upper()
                etf=(r.get('ETF') or '').strip().upper(); test=(r.get('Test Issue') or '').strip().upper()
...
def chart(sym):
    try:
        j=json.loads(get(f'https://query1.finance.yahoo.com/v8/finance/chart/{sym}?interval=1m&range=1d&includePrePost=true&events=div%2Csplits',10)); r=j['chart']['result'][0]
        ts=r.get('timestamp') or []; q=r['indicators']['quote'][0]; closes=q.get('close') or []; vols=q.get('volume') or []
...
def features(sym,fr=None,sess='regular'):
    ...
    finvol=pick(fr,'Volume','Current Volume'); avgvol=pick(fr,'Average Volume','Avg Volume'); finr=pick(fr,'Relative Volume','Rel Volume','Rel Volume (Intraday)')
    dayvol=max(sum(vols),finvol or 0); volratio=(dayvol/avgvol) if avgvol else None; rvol=finr if finr is not None else volratio
    high=max(prices); prox=p/high if high else 0; range15=rng(15); compress=max(0,1-min(range15,6)/6)
    accel=min(max(vacc,0),5)/5; rv=min(max((rvol or 0),0),3)/3; momentum=min(max(ret(5),0),6)/6
    # No baseline reward for flat/no-volume symbols. Compression only matters when flow exists.
    flow=max(accel,rv); score=100*(0.42*accel+0.33*rv+0.15*compress*flow+0.10*momentum)
    if sess=='pre-market' and dayvol>=50000 and volratio is not None: score=max(score,min(72,100*(0.55*min(volratio,0.5)/0.5+0.20*compress+0.25*momentum)))
    if change>=10:score*=0.72
    if change>=20:score*=0.55
    preflow=(sess=='pre-market' and dayvol>=50000 and (volratio or 0)>=0.06)
    early=change<10 and score>=34 and (vacc>=1.2 or (rvol or 0)>=1.25 or preflow) and range15<=5.5
    actionable=change<15 and score>=55 and prox>=0.90 and ret(5)>=-1.5
    confirmed=score>=66 and ret(5)>0.4 and (v5>0 or dayvol>=100000)
    stage='CONFIRMED' if confirmed else 'ACTIONABLE' if actionable else 'EARLY' if early else 'WATCH'
...
def main():
    ...
    frs=finviz_rows(); fmap={(r.get('Ticker') or '').strip().upper():r for r in frs}; hot=list(fmap)
    retained=[s for s,v in state['symbols'].items() if v.get('bestStage') in ('EARLY','ACTIONABLE','CONFIRMED') or v.get('maxScore',0)>=30]
    hot=list(dict.fromkeys(hot+retained))[:340]
    directory=symbol_directory(); cursor=int(state.get('sweepCursor',0)); sweep=[]
    if directory:
        take=700; sweep=[directory[(cursor+i)%len(directory)] for i in range(min(take,len(directory)))]; state['sweepCursor']=(cursor+take)%len(directory)
    universe=list(dict.fromkeys(hot+sweep)); rows=[]
    with ThreadPoolExecutor(max_workers=28) as ex:
        fut={ex.submit(features,s,fmap.get(s),sess):s for s in universe}
        for f in as_completed(fut):
            x=f.result()
            if x:rows.append(x)
    rank={'WATCH':0,'EARLY':1,'ACTIONABLE':2,'CONFIRMED':3}
    for x in rows:
        s=x['symbol']; rec=state['symbols'].setdefault(s,{'firstSeenUTC':t.isoformat(),'bestStage':'WATCH','firstChangePct':x['changePct'],'maxScore':0}); old=rec.get('bestStage','WATCH')
        ...
        if rank[x['stage']]>rank.get(old,0):
            rec['bestStage']=x['stage']; rec['stageFirstUTC']=t.isoformat(); rec['stageFirstChangePct']=x['changePct']; state['events'].append({'ts':t.isoformat(),'symbol':s,'event':'STAGE_UP','from':old,'to':x['stage'],'changePct':x['changePct'],'score':x['score']})
